...
class CartService(BaseService[ShoppingCart, CartCreate, CartUpdate]):
    ...
    async def create(self, db: AsyncSession, obj_in: CartCreate) -> ShoppingCart:
        ...
        await self.business_rules.validate(obj_in.model_dump(), db)
    ...
    async def calculate_cart_total(
        ...
        for item in cart_items:
            product = await self.product_crud.get(db, id=item.proxy_product_id)
    ...
    async def remove_cart_item(
        ...
        await self.crud.remove(db, id=cart_item_id)
        return True
